...
class CoreSystem {
    constructor(options, db, logger) {
        ...
        this.db = db;
        this.namespace = (0, utils_1.sanitizeIdentifier)(options.namespace);
        this.logger = logger;
        this.queueName = `core_events_${this.namespace}`;
        this.deadLetterQueueName = `${this.queueName}_dlq`;
    }
    static async connect(options) {
        const logger = options.logger ?? new logger_1.ConsoleLogger(`core:${options.namespace}`);
        const database = await database_1.CoreDatabase.connect(options.connectionString, logger);
        const system = new CoreSystem(options, database, logger);
        const initializer = new initializer_1.CoreInitializer(database, logger);
    ...
    async registerNode(registration = {}) {
        const nodeId = registration.nodeId ?? (0, node_crypto_1.randomUUID)().replace(/-/g, "").slice(0, 12);
        const cleanNodeId = (0, utils_1.sanitizeIdentifier)(nodeId);
    ...
    async createScheduledTask(definition) {
        ...
        const jobName = `${this.namespace}_${(0, utils_1.sanitizeIdentifier)(definition.name)}_${taskId}`;
